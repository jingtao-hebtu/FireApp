/**************************************************************************
           ...
           Copyright(C), 2021-2032, tju All rights reserved

 **************************************************************************
 * @file   : TCvMatQImage.cpp
 * @author : tao.jing
 * @email  : jingtao@tju.edu.cn
...
#define OPENCV_MAJOR_VER 4

#if OPENCV_MAJOR_VER == 4
        #define COLOR_CONVERT(X) cv::COLOR##_##X
#elif OPENCV_MAJOR_VER == 3
        #define COLOR_CONVERT(X) CV##_##X
#else
        #define COLOR_CONVERT(X) CV##_##X
#endif
...
namespace QtOcv {
    namespace {
        ...
        cv::Mat adjustChannelsOrder(const cv::Mat& srcMat, MatColorOrder srcOrder, MatColorOrder targetOrder)
        {
            ...
            if ((srcOrder == MCO_ARGB && targetOrder == MCO_BGRA)
                ...
                //BGRA <==> RBGA
                cv::cvtColor(srcMat, desMat, COLOR_CONVERT(BGRA2RGBA));
            }
            return desMat;
        }
    ...
    cv::Mat image2Mat(const QImage& img, int requiredMatType, MatColorOrder requriedOrder)
    {
        ...
        case 1:
            if (mat0.channels() == 3)
            {
                cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(RGB2GRAY));
            }
            else if (mat0.channels() == 4)
            {
                if (srcOrder == MCO_BGRA)
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(BGRA2GRAY));
                else if (srcOrder == MCO_RGBA)
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(RGBA2GRAY));
                else//MCO_ARGB
                    cv::cvtColor(argb2bgra(mat0), mat_adjustCn, COLOR_CONVERT(BGRA2GRAY));
            }
            break;
        case 3:
            if (mat0.channels() == 1) {
                cv::cvtColor(mat0, mat_adjustCn, requriedOrder == MCO_BGR ? COLOR_CONVERT(GRAY2BGR) : COLOR_CONVERT(GRAY2RGB));
            }
            else if (mat0.channels() == 3) {
                if (requriedOrder != srcOrder)
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(RGB2BGR));
            }
            else if (mat0.channels() == 4) {
                if (srcOrder == MCO_ARGB) {
                    ...
                    cv::cvtColor(mat0, mat_adjustCn, requriedOrder == MCO_BGR ? COLOR_CONVERT(BGRA2BGR) : COLOR_CONVERT(BGRA2RGB));
                }
                else {//RGBA
                    cv::cvtColor(mat0, mat_adjustCn, requriedOrder == MCO_BGR ? COLOR_CONVERT(RGBA2BGR) : COLOR_CONVERT(RGBA2RGB));
                }
            }
            break;
        case 4:
            if (mat0.channels() == 1) {
                if (requriedOrder == MCO_ARGB) {
                    ...
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(GRAY2RGBA));
                }
                else {//MCO_BGRA
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(GRAY2BGRA));
                }
            }
            else if (mat0.channels() == 3) {
                if (requriedOrder == MCO_ARGB) {
                    ...
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(RGB2RGBA));
                }
                else {//MCO_BGRA
                    cv::cvtColor(mat0, mat_adjustCn, COLOR_CONVERT(RGB2BGRA));
                }
            }
            else if (mat0.channels() == 4) {
                     ...
                     QImage::Format formatHint)
    {
        ...
        else if (mat.channels() == 3)
        {
            ...
            format = QImage::Format_RGB888;
            if (order == MCO_BGR)
                cv::cvtColor(mat, mat_adjustCn, COLOR_CONVERT(BGR2RGB));
#else
            format = QImage::Format_RGB32;
            cv::Mat mat_tmp;
